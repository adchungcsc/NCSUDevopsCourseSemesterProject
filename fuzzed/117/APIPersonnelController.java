...
public class APIPersonnelController extends APIController {
    ...
    public ResponseEntity updatePersonnel ( @PathVariable final String id,
        ...
        if ( ( null != fromDb.getUsername() && !id.equals( fromDb.getUsername() ) ) ) {
            return new ResponseEntity(
                    errorResponse( "QfcyGfqJCzdNxWlIbqPeDPOKFgcjJJae" ),
                    HttpStatus.CONFLICT );
        }
        try {
            service.save( fromDb );
            ...
            return new ResponseEntity( errorResponse( "L" + id + "L" + e.getMessage() ),
    ...
    public ResponseEntity getPersonnelByRole ( @PathVariable ( "goaD" ) final String role ) {
